...
export const Route = createFileRoute("/grade-2/$semesterId/")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.semester.title ?? "ثاني ثانوي"} | مختبر ميار` },
      { name: "description", content: `فصول ${loaderData?.semester.title ?? "ثاني ثانوي"}` },
    ],
  }),
  component: SemesterPage,
  notFoundComponent: () => (
    <div className="min-h-screen flex items-center justify-center">
      <Link to="/grade-2" className="text-primary font-bold">عودة لاختيار الفصل الدراسي</Link>
    </div>
  ),
});
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="container mx-auto px-4 pt-10 pb-6 text-center">
        <Link to="/grade-2" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary mb-4">
      ...
      <MiyarAssistant message={{ text: "كل فصل يحوي تجربة فريدة! اختاري ما يهمّك 🚀", mood: "happy" }} />
    </div>
  );
}
